...
from utils import create_marker_array, get_vercitces, bbox_iou, create_delete_marker
...
class Mapper(object):
    ...
    def __init__(self):
        ...
        self.marker_pub = rospy.Publisher(MARKERS_TOPIC, MarkerArray, queue_size=1)
    ...
    def mapping_callback(self, detection_msg: Detection, depth_msg: Image):
        ...
        for id, box, label, mask, score in zip(ids, boxes, labels, masks, scores):
            ...
            obj = Obj(id, vertices_world_frame, label, score)
        ...
        msg = create_marker_array(objects, WORLD_FRAME, header.stamp)
        if msg is not None:
